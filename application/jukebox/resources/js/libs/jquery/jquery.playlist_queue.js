...
;(function ($) {
    ...
    defaults = {
        ...
        onMedium: function () {}, // Fired when medium button is set.
        onSmall: function () {}, // Fired when small view button is set.
        onPlaylistMenu: function () {} // Fired when playlist menu is hit.
    };
    ...
    PlaylistQueue = function (options) {
        ...
        methods = {
            ...
            start: function () {
                ...
                this.horizontalScrollInit();	// Init horizontal scroll.
            },
            ...
            horizontalScrollInit: function() {
            	...
            	$('#'+ api.ids.queueListContainer).mCustomScrollbar({
                        scrollInertia: 150,
            ...
            horizontalScrollUpdate: function() {
                $('#'+ api.ids.queueListContainer).mCustomScrollbar('update');
            ...
            verifyInstance: function(el) {
				if ($(el).is(this.getDraggableObject())) return;
            ...
            jQueryRebindDragListener: function(el) {
				try {
					this.destructJqueryDraggable(); // clean up old bind
				} catch(e) {} 
                        ...
                        '<div id="' + api.ids.droppableWidget + '" class="droppableQueue medium empty">',
                            '<div id="'+ api.ids.playerBox +'" class="player-box">',
                    			'<div id="'+ api.ids.player +'">',
                    				'<div id="'+ api.ids.nowStreaming +'"></div>',
                    			'</div>',

                        		'<div id="'+ api.ids.playerBack +'" class="player-back"></div>',
                        	'</div>',
                        	
                    		'<div id="'+ api.ids.playerActions +'" class="player-actions">',
                    			'<ul>',
                    				'<li class="playlist-menu">',
                    					'<button type="button" class="btn btn-default btn-xs">Playlists &nbsp;<span class="glyphicon glyphicon-chevron-up"></span></button>',
                    				'</li>',
                    				'<li class="clean-list">',
                    		...
                    		'<div id="'+ api.ids.playerModes +'" class="player-modes">',
                    			'<ul>',
                    				'<li class="current"></li>',
                    				'<li class="small-list">',
                    					'<button type="button" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-tree-conifer"></span></button>',
                    				'</li>',
                    				'<li class="medium-list">',
                    					'<button type="button" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-tree-deciduous"></span></button>',
                    				'</li>',
                    			'</ul>',
							'</div>',

                        	'<div id="'+ api.ids.queueListWrapper +'" class="queue-list-wrapper">',
                        		'<div id="'+ api.ids.queueListContainer +'" class="queue-list-container">',
            ...
            subscribeListener: function() {
                var self = this,
					widget = this.getDroppableWidget(),
					resizeEvent = document.createEvent('UIEvents'),
					collapsableMode = function(remove) {
					...
					setActiveMode = function() {
						widget.find('#'+api.ids.playerModes).find('li').removeClass('active');
						
						if (widget.hasClass('medium')) {
							widget.find('#'+api.ids.playerModes).find('.medium-list').addClass('active');
						}
						
						if (widget.hasClass('small')) {
							widget.find('#'+api.ids.playerModes).find('.small-list').addClass('active');
						}
					};
                
                // Collapsable player ability.
                widget.find('#'+api.ids.playerBox).on('click', collapsableMode);
                
                // Prevents the background from being clickable and collapsable on an undesired locations.
                widget.find('#'+api.ids.nowStreaming, '#'+api.ids.playerActions, '#'+api.ids.playerModes).on('click', function(e) {
                    e.stopPropagation();
                });
                
                //empty queue manually
                widget.find('#'+ api.ids.playerActions +' li.clean-list button').on('click', function(e) {
                    self.clearQueue();
                });
                
                
                //playlist even fire.. handle from the app..
                widget.find('#'+ api.ids.playerActions +' li.playlist-menu').on('click', function(e) {
                    e.stopPropagation();
					api.onPlaylistMenu();
                });
                
                widget.find('#'+api.ids.playerModes).find('.medium-list').on('click', function(e) {
                    collapsableMode(true);
					widget.removeClass('small').addClass('medium');
                    api.onMedium();
                    setActiveMode();
                    self.horizontalScrollUpdate();
                });
                
                widget.find('#'+api.ids.playerModes).find('.small-list').on('click', function(e) {
                    collapsableMode(true);
					widget.removeClass('medium').addClass('small');
                    api.onSmall();
                    setActiveMode();
                    self.horizontalScrollUpdate();
                });
                
                setActiveMode(); // init defaults for first run.
            },
             ...
             * 
             */
            queueUpdated: function (item, toRemove, keepPlaying) {
                var queueSize = this.getSize(),
                    itemPosition = $(item).index(),
					keepPlaying = (false === keepPlaying) ? false : true;
				...
				if (null === this.getActive() && true === keepPlaying) {
            ...
            /**
             * Clears the queue list completly.
             */
            clearQueue: function () {
            	var list = this.getQueueList().find('.item-wrapper'),
					self = this;
            	
            	$.each(list, function(key, item) {
            		//self.queueUpdated(item, true, false);
                    item.remove();
            	});
                this._onEmpty();
            },
            ...
            setPlayPauseMode: function (playMode, revert, item) {
				...
				if (0 === this.getSize()) return; // no mods needs, queue is empty!!
			 ...
			 *
			 */
			getQueueSongIDs: function() {
				if (0 === this.getSize()) return []; // no mods needs, queue is empty!!
				
				var items = this.getQueueList().find('li'),
					songsList = [];
				
				for (var i = 0, len = items.length; i < len; i++) {
					songsList.push(Number($(items[i]).data('song').id));
				}
            ...
            _onEmpty: function () {
                ...
                this.horizontalScrollUpdate(); // updates horizontal scroller content.
        ...
        $.extend(publicAPI, {
             ...
             *
             */
            getViewDragHelper: __bind(function () {
				return this.view.dragHelper;
            }, methods),
             ...
             *
             */
            getQueueSongIDs: __bind(function () {
                return this.getQueueSongIDs();
            }, methods),
